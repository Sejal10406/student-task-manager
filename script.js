...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (mainTaskList) {
    mainTaskList.addEventListener("dragover", (e) => {
      e.preventDefault();
      const draggingEl = document.querySelector(".dragging");
      if (!draggingEl) return;
      const afterElement = getDragAfterElement(mainTaskList, e.clientY);
      if (afterElement == null) {
        mainTaskList.appendChild(draggingEl);
      } else {
        mainTaskList.insertBefore(draggingEl, afterElement);
      }
    });
    mainTaskList.addEventListener("drop", (e) => {
      e.preventDefault();
      saveTaskOrder();
    });
  }

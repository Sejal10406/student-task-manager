...
function evaluateRecurringTasks() {
  ...
  recurringTemplates.forEach(template => {
    // Check if we already have an active uncompleted task for this template today
    const hasActiveForToday = tasks.some(t => 
      t.masterId === template.id && 
      !t.completed && 
      t.deadline && 
      t.deadline.startsWith(todayStr)
    );

    if (hasActiveForToday) return;

    // Check last generation time (using a custom property or just last completed)
    const completedInstances = tasks.filter(t => t.masterId === template.id && t.completed).sort((a,b) => b.completedAt - a.completedAt);
    const lastCompleted = completedInstances.length > 0 ? new Date(completedInstances[0].completedAt) : new Date(template.startDate || now);

    let shouldGenerate = false;
    if (template.recurrence === 'daily') {
      shouldGenerate = now - lastCompleted >= 86400000;
    } else if (template.recurrence === 'weekly') {
      shouldGenerate = now - lastCompleted >= 7 * 86400000;
    } else if (template.recurrence === 'monthly') {
      shouldGenerate = now.getMonth() !== lastCompleted.getMonth() || now.getFullYear() !== lastCompleted.getFullYear();
    }

    if (shouldGenerate) {
      const newTask = {
        id: Date.now() + Math.random(),
        masterId: template.id,
        text: template.text,
